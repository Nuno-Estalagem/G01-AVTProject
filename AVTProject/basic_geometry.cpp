...
MyMesh createCube() {
	...
	const int a = sizeof(vertices) / sizeof(*vertices);
	const int b = sizeof(normals) / sizeof(*normals);
	const int c = sizeof(texCoords) / sizeof(*texCoords);
	const int d = sizeof(tangents) / sizeof(*tangents);

	float data[a + b + c + d];

	memcpy(data, vertices, sizeof(vertices));
	memcpy(data + a, normals, sizeof(normals));
	memcpy(data + a + b, texCoords, sizeof(texCoords));
	memcpy(data + a + b + c, tangents, sizeof(tangents));
...
MyMesh createCube(float size_x, float size_y, float size_z) {

	float allData[384];
	int index = 0;
	for (int i = 0; i < 24; i++) {
		allData[i * 4] = vertices[i * 4 + 0] * size_x;		// x
		allData[i * 4 + 1] = vertices[i * 4 + 1] * size_y;	// y
		allData[i * 4 + 2] = vertices[i * 4 + 2] * size_z;	// z
		allData[i * 4 + 3] = 1.0f;							// w
		index += 4;
	}
	for (float n : normals)		allData[index++] = n;
	for (float tc : texCoords)	allData[index++] = tc;
	for (float tg : tangents)	allData[index++] = tg;
